...
export function VercelV0Chat({ onAiResult }: VercelV0ChatProps) {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const res = await axios.post(`${CONSTANTS.API_URL}/api/ai/recommend`, {
        query,
      });
      if (res.data.success) {
        ...
        setAiResponse(UI_CONTENT.AI_CHAT.GENERIC_ERROR);
      }
    } catch (error) {
      console.error(error);
      setAiResponse(UI_CONTENT.AI_CHAT.ERROR_MESSAGE);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center w-full max-w-4xl mx-auto p-2 space-y-4"
    >
      <h1 className="text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-orange-500 to-green-500 tracking-tight text-center">
        {UI_CONTENT.AI_CHAT.TITLE}
      </h1>
      <p className="text-zinc-500 max-w-xl text-center">
        {UI_CONTENT.AI_CHAT.DESCRIPTION}
      </p>
        ...
        >
          <div className="overflow-y-auto">
            <Textarea
              ...
              placeholder={UI_CONTENT.AI_CHAT.PLACEHOLDER}
              className={cn(
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span className="text-xs text-zinc-500 dark:text-zinc-400 hidden group-hover:inline transition-opacity">
                  {UI_CONTENT.AI_CHAT.ATTACH_TOOLTIP}
                </span>
              </button>
            </div>
            <div className="flex items-center gap-2">
              ...
              {isLoading && (
                ...
                >
                  ...
                  <p className="text-[15px] text-zinc-600 dark:text-zinc-300 font-medium tracking-wide">
                    {UI_CONTENT.AI_CHAT.LOADING_MESSAGE}
                  </p>
                </motion.div>
              )}
