...
export function AddFoodPage() {
  ...
  const onSubmitHandler = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await AdminFoodService.add({
        name: data.name,
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 p-8 transition-colors">
        <div className="mb-6">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            Upload a new meal to the coconut menu
          </p>
        </div>
        ...
        <form className="flex flex-col gap-6" onSubmit={onSubmitHandler}>
          <div className="flex flex-col gap-2">
            ...
            <label
              htmlFor="image"
              className="cursor-pointer group flex flex-col items-center justify-center border-2 border-dashed border-gray-300 dark:border-slate-700 rounded-lg h-32 hover:bg-orange-50 dark:hover:bg-slate-800 hover:border-orange-400 dark:hover:border-orange-500 transition-colors relative overflow-hidden"
            >
              {image ? (
                <img
