...
export function AdminOrdersPage() {
  ...
  const loadOrders = async () => {
    try {
      const response = await AdminOrderService.list();
  ...
  useEffect(() => {
    // loadOrders only calls setState after an await; the lint rule's heuristic
    // appears to flag this because setOrders is also called synchronously in
    // statusHandler's optimistic update below, which is a separate, safe path.
    ...
    >
        ...
        <div className="flex flex-col items-end gap-3 w-full md:w-auto">
          ...
          <AnimatePresence>
            {showDatePicker && (
              ...
              >
                <div className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg p-1.5 mt-2 shadow-sm">
                  <span className="text-xs text-gray-500 dark:text-gray-400 px-2 font-semibold tracking-wider uppercase">
                    From
                  </span>
                  <input
                    type="date"
                    ...
                    className="bg-gray-50 dark:bg-slate-800 text-sm border border-gray-200 dark:border-slate-700 outline-none text-gray-700 dark:text-gray-300 rounded px-2 py-1 focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all"
                  />
      ...
      <div className="grid grid-cols-1 gap-6">
        <AnimatePresence>
          {filteredOrders.map((order, index) => (
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 p-6 flex flex-col md:flex-row gap-6 hover:shadow-md transition-shadow"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="mb-4">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <h4 className="text-gray-900 dark:text-gray-100 font-semibold truncate">
                      {order.address.firstName} {order.address.lastName}
                    </h4>
                    <div className="inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-xs text-slate-500 dark:text-slate-400 font-mono">
                      <Hash size={12} />
                  ...
                  <div className="inline-flex items-center justify-between gap-1.5 text-[13px] font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-1 rounded-md mb-2 shadow-sm">
                ...
                <div className="space-y-2 mt-4 bg-gray-50/50 dark:bg-slate-950/50 p-3 rounded-lg border border-gray-100 dark:border-slate-800">
              ...
              <div className="md:w-48 shrink-0 flex flex-col justify-between pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-gray-100 dark:border-slate-800 md:pl-6 gap-4">
                <div>
        ...
        {filteredOrders.length === 0 && !isLoading && (
          <div className="col-span-full py-12 text-center bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800">
            <Package
              size={48}
