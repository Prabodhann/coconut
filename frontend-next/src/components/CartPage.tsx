        ...
        >
          ...
          <div className="space-y-2">
            ...
            <p className="text-zinc-500 max-w-sm mx-auto">
              Looks like you haven&apos;t added any delicious coconut treats
              yet.
            </p>
          </div>
          <Button
            onClick={() => router.push("/")}
            className="bg-orange-500 hover:bg-orange-600 rounded-full px-8 py-6 text-lg shadow-lg hover:shadow-orange-500/25"
          >
            Explore Menu
          </Button>
        </motion.div>
                      ...
                      return (
                        ...
                        >
                          <div className="md:hidden flex items-center justify-between w-full">
                            <div className="flex items-center gap-4">
                              <img
                                src={item.image}
                                alt={item.name}
                                className="w-16 h-16 rounded-xl object-cover border border-zinc-100 dark:border-zinc-800"
                              />
                              <div>
                            ...
                            <div className="flex items-center gap-1 bg-zinc-50 dark:bg-zinc-800/80 rounded-lg p-1 border border-zinc-200 dark:border-zinc-700">
                              <button
                                ...
                                className="w-8 h-8 flex items-center justify-center text-zinc-600 dark:text-zinc-400 hover:text-red-500 hover:bg-white dark:hover:bg-zinc-700 rounded-md transition-colors"
                              >
                                <Minus className="w-4 h-4" />
                              </button>
                              <span className="text-sm font-semibold w-6 text-center text-zinc-900 dark:text-zinc-100">
                                {cartItems[item._id]}
                              </span>
                              <button
                                onClick={() => dispatch(addCartItem(item._id))}
                                aria-label={`Add ${item.name}`}
                                className="w-8 h-8 flex items-center justify-center text-zinc-600 dark:text-zinc-400 hover:text-green-500 hover:bg-white dark:hover:bg-zinc-700 rounded-md transition-colors"
                              >
                          ...
                          <div className="hidden md:flex col-span-2 justify-center">
                            <div className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-800/80 p-1.5 rounded-full border border-zinc-200 dark:border-zinc-700">
                              <button
                                onClick={() => handleRemove(item._id)}
                                aria-label={`Remove ${item.name}`}
                                className="w-7 h-7 flex items-center justify-center rounded-full bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 text-zinc-600 dark:text-zinc-300 hover:text-red-500 shadow-sm transition-colors"
                              >
                              ...
                              <button
                                onClick={() => dispatch(addCartItem(item._id))}
                                aria-label={`Add ${item.name}`}
                                className="w-7 h-7 flex items-center justify-center rounded-full bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 text-zinc-600 dark:text-zinc-300 hover:text-green-500 shadow-sm transition-colors"
                              >
                          ...
                          <div className="hidden md:flex col-span-2 justify-end items-center gap-4">
                            <span className="font-semibold text-orange-600 dark:text-orange-500 text-lg">
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <div className="space-y-4 text-zinc-600 dark:text-zinc-400">
                ...
                <div className="flex justify-between items-center text-lg">
                  ...
                  <b className="text-orange-600 dark:text-orange-500 text-2xl">
                    ₹{total}
                  </b>
                </div>
              </div>
            ...
            >
              <p className="text-sm text-zinc-500 mb-3">
                {UI_CONTENT.CART.PROMO_CODE_PROMPT}
              </p>
              <div className="flex gap-2">
                ...
                <Button
                  variant="secondary"
                  className="rounded-xl px-6 bg-zinc-200 dark:bg-zinc-700 hover:bg-zinc-300 dark:hover:bg-zinc-600"
                >
                  Submit
                </Button>
              </div>
