...
import { AdminFoodService, FoodService } from "@/services/api";
...
export function ListFoodPage() {
  const [list, setList] = useState<FoodItemRow[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const [editingItem, setEditingItem] = useState<FoodItemRow | null>(null);
  ...
  const openEditModal = (item: FoodItemRow) => {
    setEditingItem(item);
    setEditData({
      name: item.name,
  ...
  const handleEditImageUpload = async (
    ...
    if (file) {
      setEditImage(await fileToBase64(file));
    }
  };
  ...
  const submitEdit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsUpdating(true);
    try {
      const response = await AdminFoodService.update({
        id: editingItem._id,
        name: editData.name,
        description: editData.description,
        price: Number(editData.price),
        category: editData.category,
        ...(editImage ? { imageData: editImage } : {}),
      });
  ...
  const removeFood = async (foodId: string) => {
    try {
      const response = await AdminFoodService.remove(foodId);
      if (response.data.success) {
        toast.success(response.data.message);
        await fetchList();
      } else {
    ...
    >
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden transition-colors">
        <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-950/50 transition-colors">
          ...
          <div className="px-4 py-1.5 bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400 text-sm font-bold rounded-full shadow-sm border border-orange-200 dark:border-orange-500/30">
            {filteredList.length} Items
          </div>
        </div>

        <div className="bg-gray-50 dark:bg-slate-950 px-6 py-4 border-b border-gray-100 dark:border-slate-800 flex items-center gap-3 overflow-x-auto">
        ...
        <div className="p-6">
            ...
            {filteredList.map((item, index) => (
              ...
              >
                <div className="w-full h-56 relative overflow-hidden bg-gray-100 dark:bg-slate-800 border-b border-gray-100 dark:border-slate-800">
                  ...
                  />
                  <div className="absolute top-3 right-3 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-3 py-1.5 rounded-full shadow-md font-bold text-gray-900 dark:text-gray-100 border border-gray-200/50 dark:border-slate-700/50">
                    ₹{item.price}
                  </div>
                </div>

                <div className="p-5 flex-1 flex flex-col">
                  <h3 className="font-bold text-xl text-gray-900 dark:text-gray-100 mb-2 truncate">
                    {item.name}
                  </h3>

                  <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-3 mb-5 leading-relaxed flex-1">
                    {item.description || "No description provided."}
                  </p>

                  <div className="pt-4 mt-auto border-t border-gray-100 dark:border-slate-800 flex justify-between items-center">
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-700 dark:bg-slate-800 dark:text-gray-300">
                      {item.category}
                    </span>
          ...
          {filteredList.length === 0 && (
            <div className="py-16 text-center border-2 border-dashed border-gray-200 dark:border-slate-800 rounded-xl">
