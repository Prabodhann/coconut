  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-between px-2 w-full h-full pointer-events-none">
        <Sun
          className={`w-3 h-3 transition-opacity duration-300 ${isDark ? "opacity-30" : "opacity-0"}`}
        />
        <Moon
          className={`w-3 h-3 transition-opacity duration-300 ${isDark ? "opacity-0" : "opacity-30"}`}
        />
      </div>
    </button>
  );
